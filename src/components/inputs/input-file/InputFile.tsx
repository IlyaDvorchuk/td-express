...
const InputFile = ({ image, setImage, position }: IInputFile) => {
  ...
  return (
    <div className={`input-file ${position}`}>
      <label className={image ? 'current' : ''} onClick={() => inputFileRef.current ? inputFileRef.current.click() : undefined}>
      ...
      <input type='file' id={'Scan'} onChange={onSubmitFile} ref={inputFileRef} value={''} />
      {
